...
import { driverRegister, sendCode } from '../services/api';
...
export function RegisterScreen({ navigation }: RegisterScreenProps) {
  const { setToken, setDriver } = useAuth();
    ...
    >
      ...
      >
        <View style={styles.header}>
          <Text style={styles.title}>司机注册</Text>
          <Text style={styles.subtitle}>填写信息后等待审核通过即可接单</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.row}>
            ...
            <Pressable
              onPress={onSendCode}
              style={[styles.codeBtn, codeLoading && styles.codeBtnDisabled]}
              disabled={codeLoading}
            >
              {codeLoading ? (
                <ActivityIndicator size="small" color={colors.primary} />
              ) : (
                <Text style={styles.codeBtnText}>{codeSent ? '重新获取' : '获取验证码'}</Text>
              )}
            </Pressable>
          </View>
          {errors.code ? (
